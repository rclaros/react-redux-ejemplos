...
export function getProfessorDataSuccess(professors) {
  return { type: types.LOAD_PROFESSORS_SUCCESS,professors:{data:professors.data,total:professors.total }}; 
}
...
export function getProfessorsData(page=1,_sort,_order) {
  return function (dispatch) {
    dispatch(beginApiCall()); 
    return professorApi
      .getProfessorsData(page,_sort,_order)
      .then(professors => {
        dispatch(getProfessorDataSuccess(professors)); 
      })
      .catch(error => {
        dispatch(apiCallError(error));
        throw error;
